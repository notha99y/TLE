...
def predictTLELinearReg(data, plotLines = True,getStats = True):
    ...
    mRAANUnwrapped,bRAANUnwrapped = LinearReg(dataEpoch,dataRAANUnwrapped,0,0,10001,10e-6,10e-3 )
    predRAANUnwrapped = xline*mRAANUnwrapped + bRAANUnwrapped
    mMeanAnomalyUnwrapped,bMeanAnomalyUnwrapped = LinearReg(dataEpoch,dataMeanAnomalyUnwrapped,0,0,10001,10e-6,10e-3 )
    ...
    if getStats:
        ...
        statsFile.write('BStar has a measurement mean of '+ repr(BStarMean)+ ' and stdev of '+ repr(BStarStdev)+ '\n')
        statsFile.write('Inclination has a measurement mean of '+ repr(InclMean)+ ' and stdev of '+ repr(InclStdev)+ '\n')
        statsFile.write('Eccentricity has a measurement mean of '+ repr(EccentricityMean)+ ' and stdev of '+ repr(EccentricityStdev)+ '\n')
        statsFile.write('Arguement of Perigee has a measurement mean of '+ repr(AOPMean)+ ' and stdev of '+ repr(AOPStdev)+ '\n')
        statsFile.write('RAAN has a measurement mean of '+ repr(RAANMean)+ ' and stdev of '+ repr(RAANStdev)+ '\n')
        statsFile.write('Mean Anomaly has a measurement mean of '+ repr(MeanAnomalyMean)+ ' and stdev of '+ repr(MeanAnomalyStdev)+ '\n')
...
def main(fileName,testFileName):
    ...
    mBStar,bBStar,mIncl,bIncl,mEcc,bEcc,mMeanMotion,bMeanMotion,mAOPUnwrapped,bAOPUnwrapped,mRAANUnwrapped,bRAANUnwrapped,mMeanAnomalyUnwrapped,bMeanAnomalyUnwrapped = predictTLELinearReg(data)
